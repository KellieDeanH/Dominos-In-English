...
void Domino::addPlayersCLI() {
	...
	for (size_t i = 0; i < numPlayers; ++i) {
		...
		bool playerIsAI = utils::getYesNoCin("\n  >>> Want the player to be AI controlled (S/N): ", "\nEnter S for the player to be AI controlled ,\nor N if you want a human to control you!\n\n");
...
void Domino::showBoardAndPlayerStatus() {
	...
	cout << "AI controlled: " << (getCurrentPlayer().getAIPlayer() ? "sim" : "not") << "\n" << endl;
...
void Domino::currentPlayerTurnCLI(bool firstBone) {
		...
		if (!getCurrentPlayer().getAIPlayer()) {
			...
			if (firstBone) {
				...
				if (getCurrentPlayer().playerHasDoubleBones() && !boneSelected->isDoubleBone()) {
					cout << "\nIn the first play if you have a DoubleBone you have to insert it!\n\n";
	...
	if ((newPoints > 0) && ((newPoints % 5) == 0)) {
		...
		cout << "\nHave been added " << newPoints << " points your score!\n";
		cout << getCurrentPlayer().getName() << " have now " << getCurrentPlayer().getScore() << " points!\n" << endl;
	}
